...
export default function RegisterPage() {
    ...
    const renderStep = () => {
        const steps: { [key: string]: React.ReactNode } = {
            ...
            'personalization': <PersonalizationStep onSubmit={handleRegister} onBack={() => handlePreviousStep('authSetup')} isLoading={isLoading} />,
            'success': <SuccessStep />,
        };
...
const ProfileSelectStep = ({ onNext, onBack }: { onNext: (category: string) => void, onBack: () => void }) => {
    ...
    return (
        <div className="text-center">
            <h2 className="text-2xl font-bold mb-6">Em qual desses grupos você se enquadra?</h2>
...
const AuthStep = ({ onNext, onBack }: { onNext: (data: Partial<FormData>) => void, onBack: () => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!Object.values(passwordCriteria).every(Boolean)) { alert("A senha não atende a todos os requisitos."); return; }
...
const PersonalizationStep = ({ onSubmit, onBack, isLoading }: { onSubmit: (data: Partial<FormData>) => void, onBack: () => void, isLoading: boolean }) => {
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between items-center pt-4"><button type="button" onClick={onBack} className="text-sm text-text-muted hover:text-dark-text">Voltar</button><button type="submit" disabled={isLoading} className="py-3 px-6 bg-royal-blue text-white rounded-lg font-bold disabled:bg-gray-400">{isLoading ? 'Finalizando...' : 'Finalizar Cadastro'}</button></div>
        </form>
    );
};
